...
3. source_id와 source_location은 SOURCE 헤더의 값을 정확히 복사한다.
4. evidence.raw_quote에는 근거를 확인할 수 있는 짧은 원문을 넣는다.
5. 실행 결과와 성능은 CSV/로그/결과 문서 등 실행 근거가 있을 때만 사실로 추출한다.
6. 코드 파일은 구현 방식/파라미터/알고리즘 구조의 근거가 될 수 있지만, 실행 여부나 성능의 근거로 사용하지 않는다.
7. observed finding은 직접 확인되는 패턴, inferred finding은 해석으로 구분한다.
8. temp_id는 이 배치 안에서 T001, T002처럼 유일하게 만든다.
9. 문헌 검색에 유용한 Research Concept만 추출한다. 일반 단어는 제외한다.
10. 같은 사실을 중복 Evidence로 과도하게 만들지 않는다. 이 배치에서 보고서 검증에 유용한 핵심 Evidence를 우선하며 대략 40개 이하를 목표로 한다.
11. 방법은 대략 12개, 실험은 20개, Finding은 15개, Concept은 15개 이하를 우선한다.
12. 자료가 불완전하거나 서로 충돌하면 warnings에 기록한다.
13. chunk_summary는 이 배치가 전체 연구에서 어떤 내용을 담는지 3~8문장으로 압축한다.
`,
      input: `[BATCH ${batchId}]\n${text}`,
    });
